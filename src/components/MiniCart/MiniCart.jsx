...
const MiniCart = () => {
  ...
  return (
    <AnimatePresence>
      {isOpenCart && (
        <motion.section
          className={`${styles.cartContainer} z-50 fixed right-0 top-0 h-full w-screen md:w-[50%] lg:w-[40%]  bg-gray-100 box-border p-5`}
        ...
        >
            ...
            <ul className="flex flex-col gap-2 ">
              {cart && cart.length > 0 ? (
                cart.map((item) => {
                  ...
                  return (
                    <motion.li key={productId} className="grid grid-cols-[1fr_3fr_0.5fr] bg-white p-3 rounded-md" initial={{ y: '50%', opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: '50%', opacity: 0 }} transition={{ duration: 0.6 }}>
                      ...
                      <div className="flex flex-col justify-between h-full px-2">
                        ...
                        <p className="text-xs text-gray-400 transition-all font-base hover:cursor-pointer hover:text-gray-600" onClick={() => removeFromCart(productId)}>
          ...
          {cart && cart.length > 0 && (
            <div className="absolute bottom-0 left-0 w-full p-5 bg-gray-100">
